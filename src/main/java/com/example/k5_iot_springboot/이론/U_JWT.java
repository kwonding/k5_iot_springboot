...
/*
    === JWT (JSON Web Token) ===
    : JSON 객체를 이용하여 인증 정보를 안전하게 전달하는 토큰 기반 인증 양식
    - 서버가 클라이언트(사용자)에게 발급하는 디지털 서명이 된 노큰
    ...
    1. JWT와 인증/인가
        ...
        2) 인가
            ...
            - 서버는 JWT를 해석하여 해당 사용자가 어떤 API나 리소스에 접근할 수 있는지 판단
    ...
    : 마침표를 기준으로 헤더(header).내용(payload).서명(signature)로 이루어짐
        ...
        3) 서명: header나 payload를 조합하여 주어진 비밀 키로 서명한 글
