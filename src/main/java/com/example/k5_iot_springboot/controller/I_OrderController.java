...
// === Controller 기본 어노테이션 === //
@RestController // Restful API 받을거임
@RequestMapping("/api/v1/orders") // 연결
...
public class I_OrderController {
    ...
    /* 주문 생성: 인증 주체(로그인한 사용자)의 userId를 사용 (POST: 전송하다 - 새로운 데이터 생성)*/
    @PostMapping
    // 접근제어자 반환타입 메서드명() {}
    ...
    public ResponseEntity<ResponseDto<OrderResponse.Detail>> create(
            ...
            @AuthenticationPrincipal UserPrincipal userPrincipal, // 사용자 정보 추출
...
//        return ResponseEntity.ok(response); - 둘 다 같음
