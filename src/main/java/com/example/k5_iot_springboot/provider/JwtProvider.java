...
public class JwtProvider {
    ...
    *  subject=sub(username), roles는 커스텀 클레임 */
     ...
     * 리프레시(Refresh) 토큰 생성
     * */
    public String generatedRefreshToken(String username, Set<String> roles) {
    ...
    private String buildToken(String username, Set<String> roles, long expirationMs) {
        ...
        return Jwts.builder()
                ...
                .claim(CLAIM_ROLES, roleList) // 커스텀 클레임 키에 권한 목록 저장 - payload
                .setIssuedAt(iat) // 표준 클레임에 현재 시간 설정 (발행 시간)
                .setExpiration(exp) // 현재 시간에 완료 시간을 더한 설정 (완료 시간)
//                .signWith(key, SignatureAlgorithm.HS256)
    ...
    * */
    private Claims parseClaimsInternal(String token, boolean allowClockSkewOnExpiry) {
        ...
        try {
            ...
            if (allowClockSkewOnExpiry && clockSkewSeconds > 0 && ex.getClaims() != null) {
                ...
                Date exp = ex.getClaims().getExpiration(); // 완료 시간(exp) 추출
                if (exp != null) {
                    long skewMs = clockSkewSeconds * 1000L; // 허용 오차(초)를 밀리초로 변환
                    long now = System.currentTimeMillis();
                    if (now - exp.getTime() <= skewMs) {
                        // 현재 시각 - 만료 시각 <= 허용오차 범위 이내면 "방금 완료" 간주
    ...
    public boolean isValidToken(String tokenWithoutBearer) {
            ...
            // parser.parseSignedClaims(token) - 얘가 해당 내용 처리함
        try {
