...
// 기본 생성자를 생성 - 외부 new 방지, JPA 프록시/ 리플렉션용
...
//    /* 여러 권한 보유 */
//    @ElementCollection(fetch = FetchType.LAZY) // JWT에 roles를 저장하는 구조 - LAZY 기능
//    @CollectionTable(
//            name = "user_roles",
...
//    public void removeRole(RoleType role) { this.roles.remove(role); }
    ...
    public void revokeRole(G_Role role) {
        // 현재의 권한을 순회하여 매개변수로 삭제할 권한(매개변수 값)이 존재하면 삭제
        // , 그렇지 않으면 삭제 X
    ...
    public Set<RoleType> getRoleTypes () {
        return userRoles.stream()
                ...
                .collect(Collectors.toUnmodifiableSet()); // 수정 할 수 없는 Set
    }
}
